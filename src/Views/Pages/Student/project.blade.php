                    ...
                    @forelse($sprints ?? [] as $sprintGroup)
                        ...
                        <div>
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <span class="text-xs font-mono text-slate-400">{{ $sprint->get_start_date() }} —
                                    {{ $sprint->get_end_date() }}</span>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                @forelse($briefs as $briefGroup)
                                    ...
                                    <a href="/student/brief/{{ $brief->get_id() }}"
                                        class="group block bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all flex flex-col h-full">
                                        <div class="p-6 flex-1">
                                            <div class="flex items-start justify-between mb-4">
                                                ...
                                                <span
                                                    class="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border {{ $badgeClass }}">
                                                    {{ $badgeLabel }}
                                                </span>
                                            </div>

                                            <h4
                                                class="font-bold text-slate-800 mb-2 group-hover:text-indigo-600 transition-colors">
                                                {{ $brief->get_title() }}</h4>
                                            <p class="text-sm text-slate-500 line-clamp-3">{{ $brief->get_description() }}
                                            </p>
                                        </div>

                                        <div
                                            class="px-6 py-4 border-t border-slate-50 bg-slate-50/50 rounded-b-xl flex items-center justify-between">
                                            <div class="flex items-center gap-2 text-xs text-slate-400">
                                                <i data-lucide="clock" class="w-3 h-3"></i>
                                                <span>Deadline:
                                                    {{ date('M d', strtotime($brief->get_date_remise())) }}</span>
                                            </div>
                                            <span
                                                class="text-xs font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                                ...
                                @empty
                                    <div
                                        class="col-span-full py-8 text-center text-slate-400 text-sm border-2 border-dashed border-slate-100 rounded-xl">
