...
@section('content')
    <div class="flex h-screen bg-slate-50 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
            ...
            <header class="h-16 bg-white border-b border-slate-200 flex items-center gap-4 px-8">
                <a href="/student/briefs"
                ...
                <div>
                    <h2 class="text-lg font-bold text-slate-800">{{ $brief['brief']->get_title() }}</h2>
                    <div class="flex items-center gap-2 text-xs text-slate-500">
                        <span class="font-medium text-indigo-600">{{ $sprint->get_name() }}</span>
                        <span>•</span>
                        <span>Due: {{ date('M d, Y - H:i', strtotime($brief['brief']->get_date_remise())) }}</span>
                    </div>
                </div>
            </header>
                                    ...
                                    <div class="p-4 flex items-start gap-4 hover:bg-slate-50 transition-colors">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                ...
                                                Target: <strong class="ml-1 text-indigo-600">Level
                                                    {{ $comp->get_level() ?? '?' }}</strong>
                                            </div>
                                        </div>
                                    </div>
                                @empty
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 sticky top-6">
                            ...
                            @if ($status == 'done')
                                <div class="text-center py-4">
                                    ...
                                    <div class="mt-6 pt-6 border-t border-slate-100">
                                        ...
                                        <p class="text-sm text-slate-600 italic">"Check your evaluation tab for detailed
                                            feedback."</p>
                                    </div>
                                </div>
                            @elseif($status == 'submitted')
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-sm text-slate-500 mt-1">Submitted on {{ date('M d, H:i') }}</p>
                            ...
                            @else
                                <div class="text-center py-4">
                                    ...
                                    <h4 class="text-xl font-bold text-slate-800">Submit Your Work</h4>
                                    <p class="text-sm text-slate-500 mt-1">Ready to turn in your project?</p>
                                    ...
                                    <form action="/student/brief/submit" method="POST" class="mt-6 text-left">
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                                <label class="block text-xs font-bold uppercase text-slate-500 mb-1">GitHub
